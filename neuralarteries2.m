...
% Basilar Artery
ba_radius = 0.003448/2; %m %[0.003 0.004 0.005 0.006 0.007]; % artery diameter varies
...
pca_radius = 0.001633/2; %m %[0.0005 0.001 0.0015 0.002 0.0025 0.003 0.0035 0.004 0.0045 0.005]; % m
...
% pca_pressure_hyp = (pca_pressure_hyp - mean(pca_pressure_hyp)); %/1.0e+03;
...
%% Basilar Artery
% local variables
rad = linspace(-ba_radius,ba_radius, grid);
t = 2;

% normal
[p0,pn,phi] = FourierSeries(ba_pressure_norm,dt,4,1/freq);
[ba_n_u,ba_n_p,ba_n_ta,ba_n_q,dq,alpha]=PulsatileFlow(ba_radius,rho,mu,freq,p0,pn,phi,timestep,grid);
...
% pt=p0;
% for n=1:4
%     pt=pt+pn(n)*cos(2*pi*tt*n/(1/freq)-phi(n));
% end
% %plot(tt,pt,'-k',0:dt:((1/freq)-dt),ba_pressure_norm,'x')
% plot(tt,pt)
% hold on
% plot(tt,ba_pressure_norm)
% legend('p(t)','input values')

% hypertensive
[p0,pn,phi] = FourierSeries(ba_pressure_hyp,dt,4,1/freq);
[ba_h_u,ba_h_p,ba_h_ta,ba_h_q,dq,alpha]=PulsatileFlow(ba_radius,rho,mu,freq,p0,pn,phi,timestep,grid);
% velocity feild definitions
...
%     end
%     subplot(1,2,2)
    title('Basilar Artery: Velocity throughout the Artery under Normal Conditions')
    xlabel('Velocity [m/s]')
    ylabel('Diameter [m]')     
    for x = 1:grid
...
%     end
%     subplot(1,2,2)
    title('Basilar Artery: Velocity throughout the Artery under Hypertensive Conditions')
    xlabel('Velocity [m/s]')
    ylabel('Diameter [m]')    
    for x = 1:grid
...
% For this function to correctly work, u would need to be calculated again
% for a radial direction so that the n_u2 variable could be a
% 'y-coordinate' rather than the current "Cheat" system I am using
...
%% Posterior Cerebral Artery
% local variables
rad = linspace(-pca_radius,pca_radius, grid);
t = 2;

% normal
[p0,pn,phi] = FourierSeries(pca_pressure_norm,dt,4,1/freq);
[pca_n_u,pca_n_p,pca_n_ta,pca_n_q,dq,alpha]=PulsatileFlow(pca_radius,rho,mu,freq,p0,pn,phi,timestep,grid);
% velocity feild definitions
...
% hypertensive
[p0,pn,phi] = FourierSeries(pca_pressure_hyp,dt,4,1/freq);
[pca_h_u,pca_h_p,pca_h_ta,pca_h_q,dq,alpha]=PulsatileFlow(pca_radius,rho,mu,freq,p0,pn,phi,timestep,grid);
% velocity feild definitions
...
%     end
%     subplot(1,2,2)
    title('Posterior Cerebral Artery: Velocity throughout the Artery under Normal Conditions')
    xlabel('Velocity [m/s]')
    ylabel('Diameter [m]')
    for x = 1:grid
...
%     end
%     subplot(1,2,2)
    title('Posterior Cerebral Artery: Velocity throughout the Artery under Hypertensive Conditions')
    xlabel('Velocity [m/s]')
    ylabel('Diameter [m]')    
    for x = 1:grid
